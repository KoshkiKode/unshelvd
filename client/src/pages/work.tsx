  ...
  if (error || !data) {
    return (
      <div className="container mx-auto max-w-4xl px-4 py-8 text-center">
        ...
        <Link href="/browse"><Button variant="outline">Browse Books</Button></Link>
      </div>
    );
  }

  const { work, catalogEditions, userListings, languages, totalEditions, totalListings } = data;
  const allListings = Object.values(userListings).flat();
  const dir = getTextDirection(work.originalLanguage);

  return (
    <div className="container mx-auto max-w-5xl px-4 py-8" data-testid="work-page">
      <Link href="/browse">
      ...
      <div className="flex gap-6 mb-8">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="font-serif text-2xl font-bold mb-1" data-testid="work-title">
            {work.title}
          </h1>
          {work.titleOriginalScript && work.titleOriginalScript !== work.title && (
            <p
          ...
          <p className="text-muted-foreground mb-3" data-testid="work-author">
            by {work.author}
            {work.authorOriginalScript && work.authorOriginalScript !== work.author && (
              <span className="ml-2 text-sm" dir={dir}>({work.authorOriginalScript})</span>
            )}
          </p>
      ...
      <Tabs defaultValue="listings">
        <TabsList className="mb-4">
          <TabsTrigger value="listings">
            For Sale ({totalListings})
          </TabsTrigger>
          {languages.sort().map((lang) => (
            <TabsTrigger key={lang} value={`lang-${lang}`}>
              {lang} ({(catalogEditions[lang]?.length || 0) + (userListings[lang]?.length || 0)})
            </TabsTrigger>
          ))}
        </TabsList>
          ...
          {allListings.length === 0 ? (
            <Card>
              <CardContent className="py-8 text-center">
                ...
                <p className="text-muted-foreground">No copies currently for sale.</p>
                <p className="text-sm text-muted-foreground mt-1">Be the first to list a copy of this work.</p>
              </CardContent>
            </Card>
          ) : (
          ...
          return (
            <TabsContent key={lang} value={`lang-${lang}`}>
              ...
              {catEditions.length === 0 && langListings.length === 0 && (
                <p className="text-sm text-muted-foreground py-4">No editions found in {lang}.</p>
              )}
            </TabsContent>
          );
        })}
